...
## Yeni adım (phase-61)
- Program kapanışı sonrası operasyonel doğrulama için `MIGRATION_LIVE_EVIDENCE_TEMPLATE.md` eklendi.
- Dry-run, canlı cutover, 72 saat stabilizasyon ve legacy decommission kanıtlarının standart formatta toplanması hedeflendi.
- Böylece dokümantasyon programından canlı operasyon kanıt programına geçiş için ortak bir şablon oluşturuldu.



## Yeni adım (phase-62)
- Post-program operasyon kanıt sürecini hızlandırmak için `backend/scripts/generate_live_evidence_stub.py` eklendi.
- Script, `MIGRATION_LIVE_EVIDENCE_TEMPLATE.md` şablonundan zaman damgalı bir evidence dosyası (`migration-evidence/live-evidence-*.md`) üretir.
- Böylece canlı cutover/stabilizasyon kanıtlarının standart formatta açılması otomatik hale getirildi.



## Yeni adım (phase-63)
- Post-program kanıt kalitesini otomatik kontrol etmek için `backend/scripts/validate_live_evidence.py` eklendi.
- Script, live evidence dosyasındaki zorunlu alanların varlığını ve boş/todo kalmadığını doğrular.
- Böylece evidence toplama sürecinde manuel kontrol yükü azalırken operasyonel kapanış kalitesi artar.



## Yeni adım (phase-64)
- Post-program operasyon akışını hızlandırmak için `backend/scripts/open_and_validate_live_evidence.py` eklendi.
- Script, evidence stub üretimi ile ilk doğrulamayı tek komutta birleştirir.
- Böylece canlı kanıt sürecinde komut zinciri sadeleştirilerek uygulama hatası riski azaltıldı.



## Yeni adım (phase-65)
- Post-program evidence sürecinde toplu görünürlük için `backend/scripts/evidence_status_report.py` eklendi.
- Script, evidence klasöründeki tüm `live-evidence-*.md` dosyalarını doğrulayıp valid/invalid özet raporu üretir.
- Böylece operasyon ekipleri kapanış öncesi evidence bütünlüğünü tek komutta kontrol edebilir.



## Yeni adım (phase-66)
- Evidence görünürlüğünü ekip paylaşımına uygun hale getirmek için `backend/scripts/export_evidence_summary_md.py` eklendi.
- Script, `evidence_status_report.py` çıktısını markdown rapora (`backend/reports/evidence_status_summary.md`) çevirir.
- Böylece operasyon toplantılarında evidence durumunun okunabilir özeti tek dosyadan takip edilebilir.
